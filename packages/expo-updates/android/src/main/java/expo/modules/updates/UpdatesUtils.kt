...
object UpdatesUtils {
  ...
  @Throws(NoSuchAlgorithmException::class, IOException::class)
  fun sha256AndMaybeVerifySignedHashAndWriteToDestinationFile(inputStream: InputStream?, destination: File, codesigningConfiguration: Crypto.CodeSigningConfiguration?): ByteArray {
    DigestInputStream(inputStream, MessageDigest.getInstance("SHA-256")).use { digestInputStream ->
      ...
      if (codesigningConfiguration != null) {
        val isSignatureValid = codesigningConfiguration.verifyDigest(md.digest())
        if (!isSignatureValid) {
  ...
  fun maybeVerifySignedHash(inputStream: InputStream?, codesigningConfiguration: Crypto.CodeSigningConfiguration?) {
    if (codesigningConfiguration == null) {
      return
    }
    ...
    DigestInputStream(inputStream, MessageDigest.getInstance("SHA-256")).use { digestInputStream ->
      // if codesigning is enabled, ensure the signed message digest that is included with the file is valid
      val isSignatureValid = codesigningConfiguration.verifyDigest(digestInputStream.messageDigest.digest(),)
      if (!isSignatureValid) {
